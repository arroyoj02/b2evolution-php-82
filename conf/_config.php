...
{ 
    ...
    { // NOT an upgrade
        header('HTTP/1.0 503 Service Unavailable');
        readfile(__DIR__.'/umaintenance.html');
        die();
    }
}
...
// basic settings
if( file_exists(__DIR__.'/_basic_config.php') )
{   // Use configured base config:
    require_once __DIR__.'/_basic_config.php';
}
else
{   // Use default template:
    require_once __DIR__.'/_basic_config.template.php';
}

// DEPRECATED -- You can now have a _basic_config.php file that will not be overwritten by new releases
if( file_exists(__DIR__.'/_config_TEST.php') )
{ // Put testing conf in there (For testing, you can also set $install_password here):
    include_once __DIR__.'/_config_TEST.php';    // FOR TESTING / DEVELOPMENT OVERRIDES
}
...
require_once __DIR__.'/_advanced.php';           // advanced settings
require_once __DIR__.'/_locales.php';            // locale settings
require_once __DIR__.'/_formatting.php';         // formatting settings
require_once __DIR__.'/_stats.php';              // stats/hitlogging settings
require_once __DIR__.'/_application.php';        // application settings

global $app_pro;
if( isset( $app_pro ) && $app_pro === true )
{   // Load social settings only for PRO version:
    require_once __DIR__.'/_social.php'; // social settings
}

if( file_exists(__DIR__.'/_local.php') )
{ // Override for local config in there:
    include_once __DIR__.'/_local.php';           // Will not be overridden on upgrade.
}
elseif( file_exists(__DIR__.'/_overrides_TEST.php') )
{ // Legacy file (not recommended):
    include_once __DIR__.'/_overrides_TEST.php';  // Will not be overridden on upgrade.
}
